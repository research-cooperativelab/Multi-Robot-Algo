...
\begin{abstract}
Coordinating autonomous drones in disaster scenarios---collapsed buildings,
earthquake rubble, flood sites---requires assigning a resource-constrained fleet
to search an uncertain environment as efficiently as possible.
We present a unified competitive-ratio framework for this problem, wherein $R$
robots must locate a target hidden among $n$ candidate sites under a finite
per-robot energy budget~$E$ and prior probability vector~$\mathbf{p}$.
We formalize four base models: M1, in which each robot independently selects a
random unvisited site with \emph{no inter-robot communication whatsoever} (the
total uncoordinated baseline); M2, which introduces Sequential Single-Item (SSI)
auctions with unconstrained energy; M3, which adds the energy constraint with a
$p_i/d$ bid function; and M4/M4*, which extends each sortie via a greedy chain
using either $p_i/d$ or the novel $p_i/d^2$ bid, motivated by a cost-foreclosure
argument, alongside a Hungarian assignment baseline.
All bounds are validated through 5,000 Monte Carlo trials.
Four results emerge: auction coordination achieves a $6.6\times$ FCR reduction
over the uncoordinated baseline; SSI auctions with probability bids outperform
Hungarian assignment by $8.1\%$, while adding probability weighting to Hungarian
alone \emph{worsens} performance, showing that iterative bid updates are essential;
multi-node greedy chains recover $90\%$ of the energy penalty; and the $p_i/d^2$
bid achieves the best FCR across all energy-constrained configurations, with its
advantage growing to ${\sim}12\%$ at tight budgets.
Therefore, this work provides a reproducible foundational benchmark, applicable to
any scenario in which a team of resource-constrained agents must locate a target
under uncertainty, from urban search-and-rescue to planetary exploration.
\end{abstract}
...
\textbf{M1 (Random):} Robots have no communication; each independently
picks a random unvisited site and flies there, forming the uncoordinated
worst case.
\textbf{M2 (Coordinated, no battery limit):} Robots communicate at all
times and run a \emph{Sequential Single-Item (SSI) auction} to decide who
goes where, but the battery constraint is ignored, giving the theoretical
best case for coordination alone.
\textbf{M3 (Coordinated, battery-limited):} Same SSI auction as M2, but
each robot can only make one short round-trip per turn---the realistic
baseline with energy constraints.
\textbf{M4 / M4*:} The auction assigns each robot an anchor site, and
then each robot chains additional nearby sites into the same trip via a
\emph{greedy chain extension} before returning to base; M4 uses the
standard $p/d$ bid, and M4* uses our improved $p/d^2$ bid.
...
Five thousand Monte Carlo trials at the default configuration ($n{=}30$,
$R{=}3$, $E{=}14$) yield four principal findings.
\textbf{Coordination gain.}
Auction coordination alone reduces FCR by $6.6\times$ over uncoordinated random
search (M2 vs.\ M1), confirming that \emph{communication mechanism} is the
dominant design lever.
\textbf{Mechanism matters.}
Probability-aware SSI auctions outperform the centralized distance-optimal
Hungarian assignment by $8.1\%$ (M3 vs.\ $\mathrm{H}_d$); a key ablation reveals
that adding probability-weighted bids to Hungarian assignment \emph{worsens}
performance---the SSI mechanism's iterative reassignment, not the bid function
alone, is what enables effective target-directed search.
\textbf{Energy recovery.}
Multi-node sorties (M4) recover $90\%$ of the FCR gap between the
battery-constrained (M3) and battery-free (M2) settings, nearly eliminating the
battery penalty with smarter scheduling.
\textbf{Bid function.}
Our novel $p/d^2$ bid function achieves the \emph{best FCR} of all models with
energy constraints; the advantage is $1.5\%$ at the default configuration and
grows to ${\sim}12\%$ as $E$ decreases, where cost-foreclosure is most active.
...
\textbf{Four formally defined base models} (M1--M4*) spanning the
...
\textbf{Empirical demonstration} that probability-aware distributed
...
\textbf{A novel $p_i/d^2$ bid function} with a formal cost-foreclosure
...
\textbf{A fully open-source, reproducible benchmark}---the
...
These are intentional base models: they establish provable performance
guarantees and a reproducible benchmark on which future, more realistic
algorithms can be built.
They do not require full knowledge of the target's location---only the
prior distribution~$\mathbf{p}$.
...
\begin{equation}
  ...
  \label{eq:fcr}
\end{equation}
Notice that when $\mathrm{FCR} = 1$, the discovering robot traveled the shortest
possible path from its base --- the algorithm is optimal for that instance.
Values greater than~$1$ quantify the efficiency loss due to uncertainty and
