...
achieves the lowest FCR across all bid functions in both the energy and
fleet-size sweeps, improving upon $p_i/d$ by $4.2\%$ at the default
configuration (500 trials), with strongest support from the parametric
sweep results and the theoretical foreclosure argument.
...
probability-weighted Hungarian ablation that isolates the contribution of
the bid function from the allocation mechanism, showing that the bid
function is the primary driver of the observed performance difference.
...
Das et al.~\cite{Das2017} studied energy-constrained graph search,
establishing that an agent's ability to make progress is fundamentally
bounded by its return-trip cost budget---the same constraint that
distinguishes M3 and M4 from M2 in our hierarchy.
The widening M3-to-M4 performance gap as $E$ decreases in our experiments
is consistent with this energy-budget sensitivity.
...
Bourgault et al.~\cite{Bourgault2003} applied information-theoretic criteria
...
1,000 trials, indicating the aggregate mean gap at the default
configuration is within noise; the cost-foreclosure advantage is better
supported by the energy sweep than by the single-configuration point
estimate. These supplementary results are archived at
\url{https://github.com/foojanbabaeeian/Multi-Robot-Algo}.
...
Deep reinforcement learning (DRL) methods for multi-robot search
can optimize richer posterior objectives (mutual information,
Fisher information) and represent complex spatial patterns
...
Corollary~\ref{cor:foreclosure}, achieves a $4.2\%$ mean improvement over
the standard $p/d$ bid at the default configuration and constitutes the
best-performing bid function in the energy and fleet-size sweeps;
the advantage is most strongly supported by the parametric sweep results
and the theoretical foreclosure argument rather than by the
aggregate default-configuration mean alone.
