...
\title{Multi-Robot Search and Rescue Under Energy Constraints: A Competitive Analysis}
...
Tightness is the ratio of the empirical mean to the theoretical bound; a
value close to~1 means the bound is nearly tight (hard to improve), while a
low value means the bound is conservative and could be strengthened.
The M2 bound achieves tightness~$0.85$, reflecting its simple per-hop cost
structure---close to tight.
The M4 bound is most conservative (tightness~$0.22$): the analysis uses a
worst-case $S{=}1$ site per sortie, but empirically the greedy chain
visits ${\sim}5.3$ sites per trip (robots preferentially chain nearby sites
shorter than $d_{\mathrm{hop}}$).
