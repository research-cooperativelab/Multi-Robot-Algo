...
budget (mean improvement $4.2\%$ at the default configuration).
\end{abstract}
...
with empirical mean $3.11$, yielding tightness~$\mathbf{0.65}$---a $3{\times}$
improvement over the prior tightness of~$0.22$.
\end{proof}
...
fleet size reduces FCR from $18.55$ to $2.83$---a $6.6\times$ improvement.
...
iteration.
This tells us something important: optimizing for geometry alone is the
wrong objective.
The probability-weighted ablation $\mathrm{H}_{p/d}$ achieves FCR$=6.31$,
which is statistically indistinguishable from M3 ($1.4\%$ mean gap,
$0.1\%$ median gap)---confirming that the \emph{bid function}, not the
centralized-vs.-distributed mechanism, is what drives the advantage.
As a cross-check, the $1/d$ bid within the SSI auction produces
FCR$=6.79$, matching $\mathrm{H}_d$ ($6.80$) to within numerical
precision, validating both implementations.

\textbf{Result~3: Multi-node sorties recover the energy penalty.}
The gap between M3 (FCR$=6.41$, energy-constrained) and M2 (FCR$=2.83$,
unconstrained) represents the cost of finite batteries.
M4 (FCR$=3.11$) closes $92\%$ of this gap---without any additional hardware
...
with the cost-foreclosure argument of Corollary~\ref{cor:foreclosure}.
Fig.~\ref{fig:bids} confirms the complete bid-function ordering
$p/d^2 > p/d > 1/d > p{\cdot}e^{-d/E} > p$ across all 500 trials.
The advantage grows with the energy sweep (Fig.~\ref{fig:energy}): M4*
widens its lead over M4 as $E$ decreases, precisely the tight-budget
regime where cost-foreclosure is most active.
A paired Wilcoxon test at 1,000 trials yields $p{=}0.88$ at the single
default configuration, indicating the aggregate mean gap is not
individually significant at that point; the compelling evidence for
the $p/d^2$ advantage lies in the energy sweep and bid-function
comparison rather than the single-parameter-point mean.
...
M4 (prior)       &  3.11 & 13.87 & $\checkmark$ & 0.22 \\
M4 (tightened)   &  3.11 &  4.78 & $\checkmark$ & \textbf{0.65} \\
\bottomrule
\end{tabular}
\end{table}
...
The \emph{original} M4 bound (tightness~$0.22$) was the weakest: it used
...
The \emph{tightened} M4 bound (Theorem~\ref{thm:m4}, tightness~$\mathbf{0.65}$)
corrects both sources: replacing $d_{\mathrm{hop}}$ with $d_{\mathrm{nn}}$
and applying the partial-sortie decomposition raises tightness from $0.22$
to~$0.65$---a $3{\times}$ improvement.
...
$2.98$ to $3.18$ (a $6.58\%$ degradation), despite shortening total tour length.
...
Theorem~\ref{thm:m4} improves tightness from $0.22$ to $0.65$ by replacing
