...
def generate_instance(n, R, L, E, d_min, seed):
    ...
    raise ValueError("No feasible instance found")
\end{verbatim}}
...
\paragraph{Overview.} Model 1 is a minimum-performance baseline designed
to quantify the cost of zero inter-robot coordination.
Each robot independently samples a site proportionally from the
prior distribution and executes a round trip to inspect it.
Because robots exchange no information, two robots may independently
select the same site in the same round.
This is the no-coordination floor---the baseline against which every
coordinated model is measured.
...
\paragraph{Overview.} Model 2 represents the opposite extreme from
Model 1: robots share full positional information and operate under
no energy constraint.
An SSI auction serves as the coordination mechanism: each robot
submits a bid for every unvisited site proportional to its
probability-to-distance ratio, and the auction greedily matches
robots to sites so that no two robots are assigned the same
destination.
Movement follows straight-line node-to-node paths with no return
to base between visits, because the energy budget is unbounded.
Model 2 therefore establishes an upper bound on the benefit of
communication alone.
The M1--M2 gap measures the value of coordination; the M2--M3 gap
measures the cost of the finite energy constraint.
...
\paragraph{Overview.} Model 3 reintroduces the energy constraint.
Each robot operates under a fixed energy budget $E$ and must return
to base after each site visit.
The auction runs at the base at the start of every round: robots
share their accumulated observations, bid on energy-feasible sites,
and are each assigned one site.
Robots then travel to their assigned site, inspect it, and return
to base.
Model 3 exposes what this thesis calls the \emph{energy penalty}:
a robot with $E = 14$ assigned a site at distance $5$ expends $10$
units on transit and cannot apply the remaining $4$ units to
productive search.
Summed over many sorties and rounds, this overhead is substantial.
Model 4 is designed to recover it.
...
\paragraph{Overview.} Model 4 addresses Model 3's energy overhead by
allowing each robot to chain multiple sites into a single sortie
before returning to base.
The auction continues to run at the base and assigns each robot an
initial \emph{anchor} site.
After reaching its anchor, a robot greedily selects the next
highest-bid site that is reachable within the remaining energy
budget---accounting for the return leg---and continues chaining
until no further sites are reachable, at which point the robot
returns to base.
The result is substantially improved energy utilization: a sortie
under Model 4 typically visits five to six sites instead of one.

Model 4* is identical to Model 4 except for the bid function.
Model 4 uses the classical $p_i / d$ bid, which ranks sites by
probability per unit distance; Model 4* uses the novel $p_i / d^2$
bid, which penalizes distance quadratically.
Under the quadratic rule, a site twice as far away is ranked four
times lower rather than twice lower, biasing the robot toward
compact, high-probability chains.
The theoretical motivation is the \emph{cost-foreclosure argument}
developed below.
...
def m4star_iteration(sites, p, bases, R, E, visited):
    ...
    Input:  sites    - array of n site positions
            p        - current prior probabilities (updated per round)
    ...
    pos    = [bases[r].copy() for r in range(R)]  # start at base
    E_rem  = [E] * R              # energy remaining per robot
    claimed = set()               # sites already assigned
    tours  = [[] for _ in range(R)]   # one list per robot

    # --- Phase 1: SSI auction assigns each robot one anchor site ---
    # The robot with the highest p[i]/d^2 bid wins site i.
    # Sequential SSI: a robot is removed once it wins.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=1.0\textwidth]{fig_simulation_snapshot_v2.png}
  \caption{Screenshot of the interactive \textsc{SearchFCR} web simulator
    (\url{https://searchfcr.fozhan.dev}) mid-run on a 30-site,
    3-robot instance with Model~4*.
    Sites are drawn as circles scaled by prior probability; the
    color shifts from gray (unvisited) to blue (visited this round)
    and fades for sites cleared in earlier rounds.
    The FCR timeline (bottom left) and entropy-collapse curve
    (bottom right) update in real time as the user steps through
    rounds. The sidebar panel exposes all instance parameters
    and allows instant re-seeding for paired comparisons or
    benchmarking all five models on the same
    instance.}\label{fig:simulator}
\end{figure}
...
how many \emph{rounds} the algorithm requires to find the target. A
...
times. Once the expected round count is established, converting to
total distance traveled by the finder is a matter of multiplying
by the cost of one round, which depends on the movement pattern of
the specific model. Lemma~\ref{lem:rounds}
