...
Distances are Euclidean. The arena side length $L > 0$ is a fixed
constant of the instance (throughout the experiments $L = 10$). The
position of the target is written $x_\tau$. A robot inspects a site
by physically visiting it; a visit reveals whether the target is
present with certainty (the perfect-sensor assumption, discussed in
Chapter~VI as a limitation to relax in future work). After each
search round, all robots return to base and share their observations,
so $\mathcal{V}$ below denotes the \emph{union} of sites visited by
any robot in the round, not a single robot's track. The joint prior
is then updated by Bayesian renormalization: each $i \in \mathcal{V}$
gets $p_i \leftarrow 0$, and the remaining probabilities are
renormalized:
\[
...
agent would have. Figure~\ref{fig:fcr-explained} illustrates the
definition on a small worked example.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.85\textwidth]{fig_fcr_explanation.png}
  \caption{The finder competitive ratio on a small example. A robot leaves base, checks two wrong sites (A, B), and finally reaches the target at site T. Its total travel $D_f$ divided by the straight-line base-to-target distance $d_{opt}$ gives $\mathrm{FCR} = 1.50$.}\label{fig:fcr-explained}
\end{figure}
...
The first constant is the \emph{expected base-to-site distance}
$d_{\mathrm{avg}} \approx 0.5214\,L$, which is the expected distance
between two independent uniform-random points in $[0,L]^2$~\citep{Robbins1978};
this is used to estimate the expected cost of base-to-site legs.
The second is the \emph{characteristic inter-site spacing}
$d_{\mathrm{hop}} = L / \sqrt{n}$, which arises from the regular-grid
...
\emph{H1. Communication dominates fleet size.} An auction-coordinated
...
\emph{H2. Probability-awareness dominates centralization.} A
...
\emph{H3. Multi-site sorties recover most of the energy penalty.}
...
\emph{H4. A quadratic distance penalty is empirically optimal under
